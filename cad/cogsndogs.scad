/**********
	...
	Series of functions and modules for generating shapes that mesh with common timing belts.
	Loosely derived from http://www.thingiverse.com/thing:11256 which was an excellent start
	but suffered (IMHO) from too many hard-coded "parametric" values for belt profile generation.
	Most function, module and variable names should be logical and definitions are provided where
	they may be cryptic.

	Belt profile data collected from various web sources.
...
//dog_linear(T5, 10, 10, 4); //profile, teeth, height, t_dog
//FlangedPulley();
...
Two extra points are added at +/- one degree to alleviate issue with producing non 2-manifold shapes (whatadrag)
...
NOTE ON SCALING - scaling is done with the tip and root radii coordinates so
	as to avoid altering the shape of those radii as will occur if the scale() method is called with x!=y
...
dog_ext makes a belt dog with teeth facing radially outward
	profile = the profile of the belt, T5, T2_5, etc.
...
dog_int makes a belt dog with teeth facing radially inward (like a cog)
	profile = the profile of the belt, T5, T2_5, etc.
	radius = radius of dog backing sector (mm)
	subtends = angle subtended by dog backing sector (deg)
	height = the height of the dog (mm)
...
module Cog_Pulley(profile, radius, subtends, height, t_dog) {
	//args = tooth_args(profile);
...
module PulleyTest(){
	...
	difference(){
		Cog_Pulley(T5, 12.7, 370, h_teeth, 5);
		translate([0,0,-0.05]) cylinder(r=6.55,h=h_teeth+0.1,$fn=100);
	}
}

//PulleyTest();

module FlangedPulley(){
	h_teeth = 10.6; // belt width = 7mm
	Pitch_r=12.7;
	h_hub=15;
	Shaft_r=3.1;
	...
	difference(){
		union(){
			Cog_Pulley(T5, Pitch_r, 370, h_teeth, 5);
			
			// botom flange
			cylinder(r=Pitch_r+1,h=1,$fn=100);
			translate([0,0,0.8]) cylinder(r1=Pitch_r+1, r2=Pitch_r-1,h=1,$fn=100);

			// top flange
			translate([0,0,h_teeth-1]) cylinder(r=Pitch_r+1,h=1,$fn=100);
			translate([0,0,h_teeth-1-0.8]) cylinder(r1=Pitch_r-1, r2=Pitch_r+1,h=1,$fn=100);

			// hub
			cylinder(r=Pitch_r-2,h=h_hub,$fn=100);
		} // union

		// shaft hole
		translate([0,0,-0.05])
		intersection(){
			cylinder(r=Shaft_r,h=h_hub+0.1,$fn=100);
			translate([-Shaft_r+0.9,-Shaft_r,0]) cube([Shaft_r*2+0.05,Shaft_r*2+0.05,h_hub+0.1]);
		} // intersection
		
		// setscrew
		translate([-Pitch_r,0,h_teeth+2]) rotate([0,-90,0]) #BoltHole6(depth=rPin*3);
	} // diff
} // FlangedPulley
...
dog_linear makes a straight belt dog
	profile = the profile of the belt, T5, T2_5, etc.
	teeth = number of teeth engaged in the dog
	height = the height of the dog (mm)
	t_dog = thickness of the dog annulus

**********/
module dog_linear(profile, teeth, height, t_dog) {
	args = tooth_args(profile);
...
//dog_linear(T5, 5, 10, 4);

/**********

Generates a demo of the different belt dogs available
